...
namespace bb {
    namespace cascades {
      ...
      class KeyListenerMock : public UIObjectMock {
      public:
          MOCK_METHOD1(ctor, void(UIObject* parent));
      };

      class KeyListener : public UIObject {
            Q_OBJECT
        public:
            explicit KeyListener(UIObject *parent=0) : UIObject(parent) {
              mock<KeyListenerMock>()->ctor(parent);
          };
            virtual ~KeyListener() {};
Q_SIGNALS:
